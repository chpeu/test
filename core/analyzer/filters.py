...
def check_rsi_direction_coherence(
    rsi: float,
    direction: str,
    symbol: str,
    timeframe: str,
    ...
    """
    🔥 NOUVEAU: Filtre de cohérence RSI/Direction
    Bloque les entrées contre-logiques:
    - SHORT sur RSI survendu (<35) → risque de rebond
    - LONG sur RSI suracheté (>65) → risque de correction
    ...
    Returns:
        None si valide, Dict avec raison si rejeté
    """
    # Seuils de cohérence RSI/Direction
    RSI_OVERSOLD_LIMIT = 35   # Ne pas SHORT en dessous
    RSI_OVERBOUGHT_LIMIT = 65  # Ne pas LONG au dessus
    
    # SHORT sur marché survendu = contre-logique
    if direction == 'SHORT' and rsi < RSI_OVERSOLD_LIMIT:
        reason = f"RSI incohérent: SHORT interdit sur RSI survendu ({rsi:.1f} < {RSI_OVERSOLD_LIMIT})"
        if return_reason:
            return {'reason': reason, 'symbol': symbol, 'timeframe': timeframe, 'reject_category': 'RSI_INCOHERENCE'}
        logger.debug(f"🚫 {symbol} {timeframe}: {reason}")
        return {'rejected': True, 'reason': reason, 'reject_category': 'RSI_INCOHERENCE'}
    
    # LONG sur marché suracheté = contre-logique
    if direction == 'LONG' and rsi > RSI_OVERBOUGHT_LIMIT:
        reason = f"RSI incohérent: LONG interdit sur RSI suracheté ({rsi:.1f} > {RSI_OVERBOUGHT_LIMIT})"
        if return_reason:
            return {'reason': reason, 'symbol': symbol, 'timeframe': timeframe, 'reject_category': 'RSI_INCOHERENCE'}
        logger.debug(f"🚫 {symbol} {timeframe}: {reason}")
        return {'rejected': True, 'reason': reason, 'reject_category': 'RSI_INCOHERENCE'}
