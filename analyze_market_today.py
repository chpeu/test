#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Analyse des conditions de marché d'aujourd'hui via les données SQL
"""
import psycopg2
import pandas as pd
import numpy as np
from datetime import datetime, timezone, timedelta

def analyze_market_today():
    """Analyser les conditions de marché d'aujourd'hui"""
    
    conn = psycopg2.connect(
        host="localhost",
        port=5432,
        database="trade_cursor_ml",
        user="postgres",
        password="@Cmtr1di12345"
    )
    
    print("📊 ANALYSE DU MARCHÉ - AUJOURD'HUI")
    print("=" * 80)
    
    # 1. Analyse des scans
    print("\n1️⃣ ACTIVITÉ DES SCANS:")
    print("-" * 50)
    
    scan_query = """
        SELECT 
            COUNT(*) as total_scans,
            COUNT(DISTINCT symbol) as unique_symbols,
            MIN(timestamp) as first_scan,
            MAX(timestamp) as last_scan,
            AVG(setups_found) as avg_setups_per_scan,
            SUM(setups_found) as total_setups,
            AVG(scan_duration_ms) as avg_scan_duration
        FROM scan_logs 
        WHERE DATE(timestamp) = CURRENT_DATE
    """
    
    scan_df = pd.read_sql(scan_query, conn)
    if not scan_df.empty:
        row = scan_df.iloc[0]
        print(f"   Total scans: {int(row['total_scans'])}")
        print(f"   Symboles scannés: {int(row['unique_symbols'])}")
        print(f"   Période: {row['first_scan']} → {row['last_scan']}")
        print(f"   Setups trouvés: {int(row['total_setups'])} (moyenne: {row['avg_setups_per_scan']:.1f}/scan)")
        print(f"   Durée moyenne scan: {row['avg_scan_duration']:.0f}ms")
    
    # 2. Analyse des opportunités
    print("\n2️⃣ OPPORTUNITÉS DÉTECTÉES:")
    print("-" * 50)
    
    opp_query = """
        SELECT 
            COUNT(*) as total_opportunities,
            COUNT(DISTINCT symbol) as unique_symbols,
            AVG(score) as avg_score,
            MAX(score) as max_score,
            MIN(score) as min_score,
            AVG(atr_pct) as avg_atr_pct,
            AVG(volume_ratio) as avg_volume_ratio
        FROM opportunities 
        WHERE DATE(timestamp) = CURRENT_DATE
    """
    
    opp_df = pd.read_sql(opp_query, conn)
    if not opp_df.empty:
        row = opp_df.iloc[0]
        print(f"   Total opportunités: {int(row['total_opportunities'])}")
        print(f"   Symboux concernés: {int(row['unique_symbols'])}")
        print(f"   Score moyen: {row['avg_score']:.2f} (min: {row['min_score']:.2f}, max: {row['max_score']:.2f})")
        print(f"   ATR moyen: {row['avg_atr_pct']:.3f}%")
        print(f"   Volume ratio moyen: {row['avg_volume_ratio']:.2f}x")
    
    # 3. Top symboles aujourd'hui
    print("\n3️⃣ SYMBOLES LES PLUS ACTIFS:")
    print("-" * 50)
    
    symbols_query = """
        SELECT 
            symbol,
            COUNT(*) as scan_count,
            AVG(score) as avg_score,
            MAX(score) as max_score,
            AVG(atr_pct) as avg_atr,
            COUNT(CASE WHEN score > 7.0 THEN 1 END) as high_score_count
        FROM opportunities 
        WHERE DATE(timestamp) = CURRENT_DATE
        GROUP BY symbol
        ORDER BY scan_count DESC, avg_score DESC
        LIMIT 10
    """
    
    symbols_df = pd.read_sql(symbols_query, conn)
    for _, row in symbols_df.iterrows():
        print(f"   {row['symbol']:15s}: {row['scan_count']:2d} scans | score: {row['avg_score']:.1f} (max: {row['max_score']:.1f}) | ATR: {row['avg_atr']:.3f}% | high scores: {row['high_score_count']}")
    
    # 4. Analyse de volatilité
    print("\n4️⃣ ANALYSE DE VOLATILITÉ (ATR):")
    print("-" * 50)
    
    atr_query = """
        SELECT 
            atr_pct,
            COUNT(*) as count,
            AVG(score) as avg_score
        FROM opportunities 
        WHERE DATE(timestamp) = CURRENT_DATE
        GROUP BY CASE 
            WHEN atr_pct < 0.1 THEN 'Très basse (<0.1%)'
            WHEN atr_pct < 0.2 THEN 'Basse (0.1-0.2%)'
            WHEN atr_pct < 0.4 THEN 'Normale (0.2-0.4%)'
            WHEN atr_pct < 0.6 THEN 'Élevée (0.4-0.6%)'
            ELSE 'Très élevée (>0.6%)'
        END
        ORDER BY MIN(atr_pct)
    """
    
    atr_df = pd.read_sql(atr_query, conn)
    for _, row in atr_df.iterrows():
        print(f"   {row['atr_pct']:20s}: {row['count']:3d} opportunités | score moyen: {row['avg_score']:.2f}")
    
    # 5. Analyse par heure
    print("\n5️⃣ ACTIVITÉ PAR HEURE:")
    print("-" * 50)
    
    hourly_query = """
        SELECT 
            EXTRACT(HOUR FROM timestamp) as hour,
            COUNT(*) as scans,
            COUNT(DISTINCT symbol) as symbols,
            AVG(score) as avg_score,
            SUM(setups_found) as total_setups
        FROM scan_logs 
        WHERE DATE(timestamp) = CURRENT_DATE
        GROUP BY EXTRACT(HOUR FROM timestamp)
        ORDER BY hour
    """
    
    hourly_df = pd.read_sql(hourly_query, conn)
    for _, row in hourly_df.iterrows():
        hour = int(row['hour'])
        print(f"   {hour:02d}h-{hour+1:02d}h: {row['scans']:3d} scans | {row['symbols']:2d} symboles | score: {row['avg_score']:.1f} | setups: {row['total_setups']}")
    
    # 6. Analyse des rejets
    print("\n6️⃣ RAISONS DE REJET:")
    print("-" * 50)
    
    rejections_query = """
        SELECT 
            reason,
            COUNT(*) as count,
            COUNT(DISTINCT symbol) as symbols
        FROM scan_logs 
        WHERE DATE(timestamp) = CURRENT_DATE
        AND reason IS NOT NULL
        GROUP BY reason
        ORDER BY count DESC
    """
    
    rejections_df = pd.read_sql(rejections_query, conn)
    if not rejections_df.empty:
        for _, row in rejections_df.iterrows():
            print(f"   {row['reason']:30s}: {row['count']:3d} fois ({row['symbols']} symboles)")
    else:
        print("   Aucun rejet enregistré aujourd'hui")
    
    # 7. Comparaison avec la moyenne
    print("\n7️⃣ COMPARAISON AVEC LA MOYENNE (7 derniers jours):")
    print("-" * 50)
    
    comparison_query = """
        SELECT 
            'Aujourd\'hui' as period,
            COUNT(*) as scans,
            COUNT(DISTINCT symbol) as symbols,
            AVG(score) as avg_score,
            SUM(setups_found) as setups
        FROM scan_logs 
        WHERE DATE(timestamp) = CURRENT_DATE
        
        UNION ALL
        
        SELECT 
            'Moyenne 7j' as period,
            COUNT(*) / 7.0 as scans,
            COUNT(DISTINCT symbol) / 7.0 as symbols,
            AVG(score) as avg_score,
            SUM(setups_found) / 7.0 as setups
        FROM scan_logs 
        WHERE DATE(timestamp) >= CURRENT_DATE - INTERVAL '7 days'
        AND DATE(timestamp) < CURRENT_DATE
    """
    
    comp_df = pd.read_sql(comparison_query, conn)
    for _, row in comp_df.iterrows():
        print(f"   {row['period']:12s}: {row['scans']:5.1f} scans/jour | {row['symbols']:4.1f} symboles | score: {row['avg_score']:.2f} | setups: {row['setups']:.1f}")
    
    # 8. Analyse des trades vs setups
    print("\n8️⃣ CONVERSION SETUPS → TRADES:")
    print("-" * 50)
    
    conversion_query = """
        SELECT 
            'Setups détectés' as metric,
            COUNT(*) as count
        FROM scan_logs 
        WHERE DATE(timestamp) = CURRENT_DATE
        AND setups_found > 0
        
        UNION ALL
        
        SELECT 
            'Trades exécutés' as metric,
            COUNT(*) as count
        FROM trades 
        WHERE DATE(timestamp_entry) = CURRENT_DATE
    """
    
    conv_df = pd.read_sql(conversion_query, conn)
    setups = conv_df[conv_df['metric'] == 'Setups détectés']['count'].iloc[0]
    trades = conv_df[conv_df['metric'] == 'Trades exécutés']['count'].iloc[0]
    
    print(f"   Setups détectés: {setups}")
    print(f"   Trades exécutés: {trades}")
    if setups > 0:
        conv_rate = (trades / setups) * 100
        print(f"   Taux de conversion: {conv_rate:.1f}%")
    
    conn.close()
    
    # 9. Résumé et conclusions
    print("\n🎯 RÉSUMÉ DU MARCHÉ - AUJOURD'HUI")
    print("=" * 80)
    
    if not scan_df.empty and not opp_df.empty:
        print(f"\n📈 ACTIVITÉ:")
        print(f"   • Scans: {int(scan_df.iloc[0]['total_scans'])}")
        print(f"   • Opportunités: {int(opp_df.iloc[0]['total_opportunities'])}")
        print(f"   • Trades: {trades}")
        print(f"   • Conversion: {conv_rate:.1f}%")
        
        print(f"\n📊 CONDITIONS:")
        print(f"   • Volatilité (ATR): {opp_df.iloc[0]['avg_atr_pct']:.3f}%")
        print(f"   • Score moyen: {opp_df.iloc[0]['avg_score']:.2f}")
        print(f"   • Volume: {opp_df.iloc[0]['avg_volume_ratio']:.2f}x")
        
        print(f"\n🔍 ANALYSE:")
        if opp_df.iloc[0]['avg_atr_pct'] < 0.2:
            print(f"   • Volatilité FAIBLE → moins d'opportunités")
        elif opp_df.iloc[0]['avg_atr_pct'] > 0.5:
            print(f"   • Volatilité ÉLEVÉE → plus de risques")
        else:
            print(f"   • Volatilité NORMALE → conditions idéales")
        
        if conv_rate < 20:
            print(f"   • Taux de conversion FAIBLE ({conv_rate:.1f}%)")
            print(f"     → Filtres trop stricts ou ML filter bloquant ?")
        elif conv_rate > 50:
            print(f"   • Taux de conversion ÉLEVÉ ({conv_rate:.1f}%)")
        else:
            print(f"   • Taux de conversion NORMAL ({conv_rate:.1f}%)")
        
        print(f"\n💡 RECOMMANDATIONS:")
        if opp_df.iloc[0]['avg_atr_pct'] < 0.2:
            print(f"   • Volatilité faible: envisager d'élargir les filtres")
        if conv_rate < 20:
            print(f"   • Conversion faible: vérifier les rejets ML")
        if trades < 5:
            print(f"   • Peu de trades: normal si volatilité faible")

if __name__ == '__main__':
    try:
        analyze_market_today()
    except Exception as e:
        print(f"Erreur: {e}")
        import traceback
        traceback.print_exc()
